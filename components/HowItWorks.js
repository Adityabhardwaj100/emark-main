...
export default function HowItWorks() {
  ...
  useGSAP(() => {
    ...
    // 1. PIN THE ENTIRE SECTION — works on both mobile & desktop
  ...
  return (
    <section ref={containerRef} className="relative w-full h-screen bg-black flex flex-col justify-center items-center z-10 selection:bg-white selection:text-black overflow-hidden">
               ...
               >
                  <div className="w-full h-full bg-[#080808] border border-white/20 rounded-2xl md:rounded-3xl p-5 md:p-14 shadow-[0_-20px_60px_rgba(0,0,0,0.9)] flex flex-col md:flex-row md:items-center justify-between">
                      ...
                      <div className="w-full md:w-3/5 flex flex-col items-start pr-0 md:pr-10 relative z-10 h-full justify-center">
                          ...
                          <p className="text-gray-400 text-sm md:text-lg lg:text-xl font-dm mb-5 md:mb-10 leading-relaxed max-w-lg">
                             {step.description}
                          </p>
                          
                          <div className="mt-auto md:mt-0 px-3 md:px-4 py-1.5 md:py-2 border border-gray-600 rounded-full bg-white/[0.02] inline-flex">
                             <span className="text-[9px] md:text-[10px] font-mono text-gray-300 uppercase tracking-[0.2em]">{step.metric}</span>
                          </div>
                      </div>

                      {/* Right: Brutalist Typography Overlay — desktop only */}
