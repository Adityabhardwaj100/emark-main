...
export default function HowItWorks() {
  ...
  return (
    <section ref={containerRef} className="relative w-full bg-black flex flex-col justify-center items-center z-10 selection:bg-white selection:text-black overflow-hidden py-16 md:py-0 md:h-screen">
      <div className="max-w-[1200px] w-full mx-auto px-4 md:px-6 lg:px-12 relative flex flex-col items-center">
        ...
        <div className="hidden md:block relative w-full h-[58vh] md:h-[60vh] max-h-[600px]">
          {STEPS.map((step, idx) => (
            <div
              key={step.num}
              ...
              style={{ zIndex: idx + 10 }}
            >
              <div className="w-full h-full bg-[#080808] border border-white/20 rounded-3xl p-14 shadow-[0_-20px_60px_rgba(0,0,0,0.9)] flex flex-row items-center justify-between">
                <div className="w-3/5 flex flex-col items-start pr-10 relative z-10 h-full justify-center">
                  ...
                  <h3 className="text-5xl font-syne font-bold text-white mb-6 uppercase tracking-tighter">{step.title}</h3>
                  <p className="text-gray-400 text-lg font-dm mb-10 leading-relaxed max-w-lg">{step.description}</p>
                  <div className="mt-auto px-4 py-2 border border-gray-600 rounded-full bg-white/[0.02] inline-flex">
                    <span className="text-[10px] font-mono text-gray-300 uppercase tracking-[0.2em]">{step.metric}</span>
                  </div>
                </div>
                <div className="flex w-2/5 justify-end items-center h-full relative pointer-events-none select-none">
                  <div className="text-[250px] font-syne font-black text-transparent absolute right-0 top-1/2 -translate-y-1/2" style={{ WebkitTextStroke: "2px rgba(255,255,255,0.03)" }}>{step.num}</div>
        ...
        <div className="flex md:hidden flex-col gap-5 w-full">
          {STEPS.map((step, idx) => (
            ...
            >
              ...
              <div className="mt-auto px-3 py-1.5 border border-gray-600 rounded-full bg-white/[0.02] inline-flex self-start">
                <span className="text-[9px] font-mono text-gray-300 uppercase tracking-[0.2em]">{step.metric}</span>
              </div>
            </div>
          ))}
        </div>
